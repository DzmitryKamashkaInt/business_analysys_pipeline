...
**Q:** [Evaluation round 3 — loop-limit checkpoint] Vision & Scope's "Required Integrations" section still blanket-denies any Bitrix24-hosted payment widget usage, contradicting UC-8's subscription flow. The automatic 2-cycle rework limit was reached. How to proceed? → **A:** Fix it and finalize — apply the one-line fix to Required Integrations (state two distinct payment paths, matching UC-8 and the architecture spec), then move both documents to ready_for_dev_docs/ as final.
