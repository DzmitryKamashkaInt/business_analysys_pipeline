...
- **Bitrix24** — OAuth/identity for admin and (per the proposal's architecture) bridged end-user auth; CRM for orders/deals (backing checkout and "My Orders"); product/catalog data; payment processing via **two distinct paths**: cart/product checkout (UC-6/FR-20) uses **Bitrix24 Payment APIs called directly (server-to-server) from the Commerce Bridge**, with a custom Rumica-built form that collects and tokenizes payment details client-side rather than the Bitrix24-hosted widget/redirect; subscription payment (UC-8/FR-21) instead uses **Bitrix24's own hosted payment flow**, a separate, intentional path from the custom checkout form. Central to nearly all MVP commerce and admin functionality.
